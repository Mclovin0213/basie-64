...
**Window translucency (shipped):** macOS launches transparent by default (`with_transparent(true)` + `fullsize_content_view` + custom panel fills). A persisted `Settings.translucent_window` (default true) is exposed in the top-bar settings menu; when off, panels paint opaque `bg_surface` and `clear_color` returns the opaque base color. The default "see through" look comes from painted `bg_window_tinted` over a transparent window. Surface tokens carry alpha (`bg_window_tinted` ~92%, `panel_glass` ~96%, `history_bg` ~92%, `bg_card` / `bg_input` ~94%) so cards and inputs pick up a hint of the window tint instead of floating as opaque stickers. Geometry is unified: window-edge panels = 11 (`WINDOW_RADIUS`), large surfaces (`card_frame` / `glass_panel` / `overlay_frame`) = 10, small chrome (inputs, banners, buttons) = 6. The central panel has a 12px left/right inner margin so input/output blocks don't kiss the window edges.

**Native macOS vibrancy (opt-in, shipped):** `Settings.experimental_native_vibrancy` (default false, toggle in the top-bar settings menu, macOS-only) installs a real `NSVisualEffectView` behind the window for genuine OS-level frosted-glass blur. `window_vibrancy::apply_vibrancy` does not work directly with eframe-wgpu — it adds the effect view as a *subview of* the Metal-backed `WinitView`, so the effect view's layer composites *above* the Metal layer and blanks the UI. The fix lives in `src/macos_vibrancy.rs`: on the first `update()` frame, walk up from the `WinitView` (raw `ns_view` from `raw-window-handle`) to its `superview`, allocate an `NSVisualEffectView` via `objc2`/`objc2-app-kit`, and call `parent.addSubview_positioned_relativeTo(&effect, NSWindowOrderingMode::Below, Some(&winit_view))` — installing the effect view as a *sibling below* the WinitView in the same parent. Material is `HUDWindow`, blending `BehindWindow`, state `FollowsWindowActiveState`, corner radius `WINDOW_RADIUS` via the CALayer. When the flag is on, `theme::Tokens::window_fill_for(translucent, native_vibrancy)` returns `Color32::TRANSPARENT` so the panel fills don't tint the OS blur, and `clear_color` returns `Rgba::TRANSPARENT`. Toggling the flag requires an app restart to take effect (AppKit can't cleanly un-install the effect view). If you re-attempt native vibrancy by calling `window_vibrancy::apply_vibrancy` directly, the UI will blank — go through `macos_vibrancy::try_install_native` instead.
...
├── settings.rs          Persisted prefs (theme, private_mode, translucent_window, experimental_native_vibrancy, recent files) — TOML in OS config dir via `directories`.
├── macos_vibrancy.rs    macOS-only (`#![cfg(target_os = "macos")]`). `try_install_native(frame, radius)` performs the manual `objc2` view-hierarchy surgery that installs an `NSVisualEffectView` as a sibling below the wgpu Metal-backed `WinitView`. Called once from `Basie64App::update` when `experimental_native_vibrancy` is true and `vibrancy_installed` is false. See the "Native macOS vibrancy" paragraph at the top of this file for why `window_vibrancy::apply_vibrancy` is bypassed.
...
| Native macOS NSVisualEffectView blur (opt-in) | `src/macos_vibrancy.rs` + `Settings.experimental_native_vibrancy` + first-frame install in `src/app.rs` (`update`) + `Tokens::window_fill_for` |
